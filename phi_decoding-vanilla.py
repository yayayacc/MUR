# bon, 用yes的token probability来作为评判结果
import os
import json
import numpy as np
import argparse
...
parser.add_argument('--file_name', type=str, default='phi_decoding-vanilla.json')
...
parser.add_argument('--scaling_rate', type=float, default=0.9, help='scaling rate for momentum uncertainty')
...
policy_model = LLM(model=policy_dir, tensor_parallel_size=1, max_model_len=8096*2,trust_remote_code=True, gpu_memory_utilization=0.9)
        ...
        try:
            ...
            if True:
                ...
                for cur_output in outputs[0].outputs:
                    ...
                    all_cumulative_logp.append(cur_output.cumulative_logprob/(len(cur_output.token_ids)+1e-8))
                ...
                sampling_params = SamplingParams(max_tokens=2048, temperature=0.6, stop=["Step"], logprobs=1)
                foresight_outputs = policy_model.generate(foresight_inputs, sampling_params)

                foresight_cumulative_logp = []
                foresight_text = []
                for foresight_idx in range(len(all_cumulative_logp)):
                    foresight_cumulative_logp.append(foresight_outputs[foresight_idx].outputs[0].cumulative_logprob/(len(foresight_outputs[foresight_idx].outputs[0].token_ids)+1e-8))
                    foresight_text.append(foresight_outputs[foresight_idx].outputs[0].text.strip())
                ...
                for text, cumulative_logp in zip(foresight_text, foresight_cumulative_logp):
                    if text != '':
                        ...
                        renewed_foresight_cumulative_logp.append(cumulative_logp)
                ...
                try:
                    ...
                    cluster_len_ratio = [len(cluster)/len(foresight_text) for cluster in cluster_list]
                    per_sample_cluster_len_ratio = [cluster_len_ratio[cluster_labels[ddi]] for ddi in range(len(foresight_text))]
                    cluster_weights = softmax(per_sample_cluster_len_ratio)

                    foresight_cumulative_logp = softmax(foresight_cumulative_logp)

                    weighted_foresight_cumulative_logp = [(foresight_cumulative_logp[i] + cluster_weights[i])/2 for i in range(len(foresight_text))]
                    best_cluster_idx = np.random.choice(range(len(foresight_text)), p=weighted_foresight_cumulative_logp)
                    best_cluster_text = foresight_text[best_cluster_idx]
                    print('success foresight at step: ', step_idx, '  ', )
                    # print('best_cluster_text: ', best_cluster_text)
                except:
                    if len(foresight_text) != 0:
                        best_candidate_idx = np.random.choice(range(len(foresight_text)), p=np.exp(foresight_cumulative_logp)/sum(np.exp(foresight_cumulative_logp)))
                        print("fail foresight at step: ", step_idx)
                    else:
                        best_candidate_idx = np.random.choice(range(len(all_cumulative_logp)), p=np.exp(all_cumulative_logp)/sum(np.exp(all_cumulative_logp)))
                        print("fail foresight at step and foresight_text is empty: ", step_idx)
