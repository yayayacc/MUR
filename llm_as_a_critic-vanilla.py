...
parser.add_argument('--file_name', type=str, default='llm_as_a_critic-vanilla.json')
    ...
    for step_idx in range(max_steps):
        try:
            ...
            if True:
                ...
                if 'math' in args.data_path.lower() or 'aime' in args.data_path.lower():
                    critic_prompt_dict = ciritique_last_generation_math(question, current_traj)
                else:
                    critic_prompt_dict = ciritique_last_generation(question, current_traj)  
                ...
                sampling_params = SamplingParams(max_tokens=4096, temperature=0.6, stop=['</analyze>\n', '```python'], include_stop_str_in_output=True)
                critic_outputs = critic_model.generate(critic_inputs, sampling_params)
                analyze_text = critic_outputs[0].outputs[0].text.strip()
                all_critic_output_tokens += len(critic_outputs[0].outputs[0].token_ids)
                output_start = "<output>\n**Judgement**: $\\boxed"
                critic_inputs = critic_inputs + analyze_text + output_start
                sampling_params = SamplingParams(max_tokens=4096, temperature=0.6, stop=['</output>\n', '</think>\n', '```python'], include_stop_str_in_output=True)
                critic_outputs = critic_model.generate(critic_inputs, sampling_params)
                ...
                analyze_text = re.search(r'<analyze>(.*?)</analyze>', critic, re.DOTALL).group(1)
                if '</think>' in critic:
                    output_text = re.search(r'<output>(.*?)</think>', critic, re.DOTALL).group(1)
                else:
                    output_text = re.search(r'<output>(.*?)</output>', critic, re.DOTALL).group(1)
                analyze_text = analyze_text.replace('<analyze>', '').replace('</analyze>', '').replace('paragraph_', 'Step').replace('paragraph', 'Step')
                analyze_text = analyze_text.replace('**Judgement**:', 'So the correctness of the step is:')
                if not 'yes' in output_text.lower():
                    ...
                    policy_inputs = [
                        ...
                        {'role': 'assistant', 'content': 'Refined Step' + str(step_idx) + ': '}
                    ]
                    ...
                    all_policy_output_tokens += len(outputs[0].outputs[0].token_ids)
                    output = outputs[0].outputs[0]
                    cur_signal = output.cumulative_logprob / (len(output.token_ids)+1e-8)
                    current_traj[-1] = 'Step' + str(step_idx) + ': ' + output.text.strip()
        except:
            pass
