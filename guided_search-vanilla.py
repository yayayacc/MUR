...
parser.add_argument('--data_path', type=str, default='data/math_500_test_new.json')
...
parser.add_argument('--verify_num', type=int, default=1, help='for each candidate, verify how many times')
parser.add_argument('--scaling_rate', type=float, default=0.8, help='scaling rate for momentum uncertainty')
...
policy_model = LLM(model=policy_dir, tensor_parallel_size=1, max_model_len=8096*2,trust_remote_code=True, gpu_memory_utilization=0.4)
critic_model = LLM(model=critic_dir, tensor_parallel_size=1, max_model_len=8096*2,trust_remote_code=True, gpu_memory_utilization=0.9)
policy_tokenizer = AutoTokenizer.from_pretrained(policy_dir, trust_remote_code=True)
critic_tokenizer = AutoTokenizer.from_pretrained(critic_dir, trust_remote_code=True)
    ...
    for step_idx in range(max_steps):
        try:
            ...
            if step_idx > 0:
                ...
                inputs = inputs.replace(policy_stop_token, "").strip() + f'\nStep0:'
            sampling_params = SamplingParams(max_tokens=2048, temperature=0.6, stop=["Step"], logprobs=1)
            ...
            current_traj.append('Step' + str(step_idx) + ': ' + output.text.strip())
            
            # if np.exp(cur_signal) < np.exp(calibrated_momentum_uncertainty)*args.scaling_rate and output.text.strip() != '' and step_idx > 0:
            if True:
                cur_step_candidates = []
                cur_normalized_logp = []
                question = tem_test_data['input']
                ...
                if step_idx > 0:
                    ...
                    inputs = inputs.replace(policy_stop_token, "").strip() + f'\nStep0:'
                sampling_params = SamplingParams(max_tokens=2048, temperature=0.6, stop=["Step"], logprobs=1, n=args.candidate_num)

                outputs = policy_model.generate(inputs, sampling_params)
                for cur_output in outputs[0].outputs:
                    cur_step_candidates.append(cur_output.text.strip())
                    cur_normalized_logp.append(cur_output.cumulative_logprob / (len(cur_output.token_ids)+1e-8))
                ...
                for candidate in cur_step_candidates:
                    ...
                    if 'math' in args.data_path.lower() or 'aime' in args.data_path.lower():
                        critic_prompt_dict = ciritique_last_generation_math(question, current_traj[:-1] + [candidate])
                    else:
                        critic_prompt_dict = ciritique_last_generation(question, current_traj[:-1] + [candidate])
                    ...
                    critic_inputs = critic_inputs.replace(critic_stop_token, "").strip()
                ...
                sampling_params = SamplingParams(max_tokens=1024, temperature=0.6, stop=['</analyze>\n', '```python'], include_stop_str_in_output=True, n=args.verify_num)
                ...
                for critic_idx, critic_output in enumerate(analyze_outputs):
                    for output in critic_output.outputs:
                        ...
                        output_inputs.append(analyze_inputs[critic_idx] + analyze_text + output_start)
                        all_critic_output_tokens += len(output.token_ids)
                sampling_params = SamplingParams(max_tokens=2048, temperature=0.6, stop=['</output>\n', '</think>\n', '```python'], include_stop_str_in_output=True, logprobs=1)
                output_outputs = critic_model.generate(output_inputs, sampling_params)

                candidate_yes_logp = [0. for _ in range(len(cur_step_candidates))]
                    ...
                    for output in critic_output.outputs:
                        ...
                        for each_item in output.logprobs:
                            ...
                            if find_yes or find_no:
                                break
                        if find_yes:
                            candidate_yes_logp[critic_idx//args.verify_num] += np.exp(yes_logp)
                ...
                current_traj[-1] = 'Step' + str(step_idx) + ': ' + best_candidate
                cur_signal = cur_normalized_logp[best_candidate_idx]
                cur_candidate_traj = {'step_idx':str(step_idx), 'step_uncertainty':str(np.exp(-cur_signal)), 'momentum_uncertainty/gamma':str(np.exp(-momentum_uncertainty)/0.9), 'selected_idx':str(best_candidate_idx),'candidates':cur_step_candidates, 'original_traj': original_traj}
                candidate_traj.append(cur_candidate_traj)
                
            momentum_uncertainty = args.momentum_rate * momentum_uncertainty + (1 - args.momentum_rate) * cur_signal
    ...
    if not get_answer:
        try:
            ...
            if 'aime' in args.data_path:
                sampling_params = SamplingParams(max_tokens=8096, temperature=0.6, logprobs=1)
            else:
                sampling_params = SamplingParams(max_tokens=8096, temperature=0.6, logprobs=1)
...
with open('res/time/' + args.file_name + '.txt', 'w') as f:
    f.write('\n\n' + args.file_name + '  time: ' + str(end_time - start_time) + '\n\n')
    f.write('all_policy_output_tokens: ' + str(all_policy_output_tokens) + '\n')
    f.write('all_critic_output_tokens: ' + str(all_critic_output_tokens) + '\n')
